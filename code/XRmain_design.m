...
type=1;  % 优化问题代号，1-32，选择对应的数字即可（自行修改）
%% 算法参数设置 Parameters-（自行修改）
nPop=70; % 种群数
Max_iter=80;% 最大迭代次数
%% 使用算法求解
run_times= 10; % 运行次数（自行修改）
...
%----------------------------------- SAA-1----------------------------------- 
tic
NodesNumber = 4; % 起点与终点之间节点的个数
dim = 2 * NodesNumber; % 维度，一组坐标点为[x, y, z]3个值，其中X等间隔分布，所以总的数据个数为2*NodesNumber
lb = [20.*ones(1, NodesNumber), 0.*ones(1, NodesNumber)]; % x, y, z的下限[20, 20, 0]
ub = [180.*ones(1, NodesNumber), 50.*ones(1, NodesNumber)]; % x, y, z的上限[200, 200, 50]
fobj = @(x)fun(x, NodesNumber, startPoint, endPoint, ThreatAreaPostion, ThreatAreaRadius); % 适应度函数
[Best_x,Best_f,cg_curve]=SAAone(nPop,Max_iter,lb,ub,dim,fobj,4);
Optimal_results{1,2}='SAA-1';
...
ub = [180.*ones(1, NodesNumber), 50.*ones(1, NodesNumber)]; % x, y, z的上限[200, 200, 50]
fobj = @(x)fun(x, NodesNumber, startPoint, endPoint, ThreatAreaPostion, ThreatAreaRadius); % 适应度函数
[Best_x,Best_f,cg_curve]=SAAtwo(nPop,Max_iter,lb,ub,dim,fobj,4);
Optimal_results{1,3}='SAA-2';
...
ub = [180.*ones(1, NodesNumber), 50.*ones(1, NodesNumber)]; % x, y, z的上限[200, 200, 50]
fobj = @(x)fun(x, NodesNumber, startPoint, endPoint, ThreatAreaPostion, ThreatAreaRadius); % 适应度函数
[Best_x,Best_f,cg_curve]=SAAthree(nPop,Max_iter,lb,ub,dim,fobj,4);
Optimal_results{1,4}='SAA-3';
...
ub = [180.*ones(1, NodesNumber), 50.*ones(1, NodesNumber)]; % x, y, z的上限[200, 200, 50]
fobj = @(x)fun(x, NodesNumber, startPoint, endPoint, ThreatAreaPostion, ThreatAreaRadius); % 适应度函数
[Best_x,Best_f,cg_curve]=ISAA(nPop,Max_iter,lb,ub,dim,fobj,22);
Optimal_results{1,5}='MSAA';         % 算法名字
...
% 将 结果 保存 mat
save (['消融实验' num2str(type) '.mat'], 'Optimal_results', 'Results','wilcoxon_test','friedman_p_value')
...
saveas(gcf,['消融实验' num2str(type)]) % 保存图窗
...
saveas(gcf,['箱线图-消融实验' num2str(type)]) % 保存图窗
